...
function CartScreen({props}) {
    const {id} = useParams()
    console.log('CartScreen-> match id:', String(id))
    ...
    console.log('In CartScreen -> product id:', id, ' qty: ', qty)
    const dispatch = useDispatch()
    const cart = useSelector(state => state.cart)
    const {cartItems} = cart
    console.log('cart items:', cartItems)
    console.log('UseParams:', String(id))


    useEffect(() => {
        if (id) {
            console.log('dispatching add to cart id:', id)
